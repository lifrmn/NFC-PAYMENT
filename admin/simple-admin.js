...
const PORT = 3000;
// Port server (3000)
const APP_SECRET = 'NFC2025SecureApp';
...
const NGROK_URL = 'https://contrite-unhappily-custodian.ngrok-free.dev';
...
class SimpleNFCAdmin {
  ...
  setupExpress() {
    ...
    this.app.use(cors({
      ...
      allowedHeaders: ['Content-Type', 'x-app-key', 'user-agent']
    ...
    // Get semua device
    this.app.post('/api/sync-device', this.syncDevice.bind(this));
    // Sync data dari HP
    this.app.post('/api/update-balance', this.updateBalanceSecure.bind(this));
    // Top-up saldo
    
    // Fraud detection endpoints
    this.app.post('/api/fraud-alert', this.handleFraudAlert.bind(this));
    // Terima fraud alert dari AI
    this.app.get('/api/fraud-alerts', this.getFraudAlerts.bind(this));
    // Get fraud alerts
    this.app.get('/api/transactions', this.getAllTransactions.bind(this));
    // Get semua transaksi
    
    // User management endpoints
    this.app.get('/api/users', this.getUsersEndpoint.bind(this));
    // Get semua user
    this.app.post('/api/users', this.createUserEndpoint.bind(this));
    // Create user baru
    this.app.put('/api/users/:id', this.updateUserEndpoint.bind(this));
    // Update user
    this.app.delete('/api/users/:id', this.deleteUserEndpoint.bind(this));
    // Delete user
    
    // Admin action endpoints
    this.app.post('/api/block-user', this.blockUserEndpoint.bind(this));
    // Block user
    this.app.post('/api/unblock-user', this.unblockUserEndpoint.bind(this));
    // Unblock user
    this.app.post('/api/bulk-topup', this.bulkTopupEndpoint.bind(this));
    // Bulk top-up
    this.app.post('/api/reset-balance', this.resetBalanceEndpoint.bind(this));
    // Reset user balance
    this.app.post('/api/clear-fraud-alerts', this.clearFraudAlertsEndpoint.bind(this));
    // Clear alerts
    
    // NFC Card management endpoints
    this.app.get('/api/nfc-cards', this.getNFCCards.bind(this));
    // Get all NFC cards
    this.app.post('/api/nfc-cards/register', this.registerNFCCard.bind(this));
    // Register new card
    this.app.post('/api/nfc-cards/link', this.linkNFCCard.bind(this));
    // Link card to user
    this.app.post('/api/nfc-cards/block', this.blockNFCCard.bind(this));
    // Block card
    this.app.post('/api/nfc-cards/topup', this.topupNFCCard.bind(this));
    // Topup card balance
    this.app.delete('/api/nfc-cards/:cardId', this.deleteNFCCard.bind(this));
    // Delete card
    ...
    this.app.get('/api/ping', (req, res) => {
      ...
      res.json({
        ...
        // Waktu sekarang
        server: 'NFC Payment Admin Server',
        // Nama server
        version: '1.0.0',
        // Versi server
  ...
  async getDevices(req, res) {
    ...
    try {
      ...
      const devices = Array.from(this.devices.values()).map(device => {
        ...
        // Device online jika sync dalam 5 menit terakhir
        const isOnline = (now - new Date(device.lastSyncAt)) < 300000;
        // 5 menit = 300000 ms
        ...
        return {
          ...
          // Platform (android)
          totalUsers: device.totalUsers || 0,
          // Total user
          totalBalance: device.totalBalance || 0,
          // Total saldo
          totalTransactions: device.totalTransactions || 0,
          // Total transaksi
  ...
  startCleanupTimer() {
    ...
    setInterval(() => {
      ...
      for (const [deviceId, device] of this.devices.entries()) {
        ...
        if ((now - device.lastSyncAt) > 600000) {
          // 10 menit = 600000 ms
  ...
  async handleFraudAlert(req, res) {
    ...
    try {
      ...
      const alertId = `fraud_${Date.now()}_${device.deviceId}`;
      // Generate ID unik
      const fraudAlert = {
        ...
        // ID transaksi
        timestamp: fraudDetection.timestamp,
      ...
      this.fraudStats.totalAlerts++;
      // Tambah total alerts
      this.fraudStats.lastAlert = new Date().toISOString();
      ...
      if (fraudDetection.decision === 'BLOCK') {
        ...
        this.fraudStats.blockedTransactions++;
        ...
        this.fraudStats.reviewTransactions++;
    ...
    try {
      ...
      for (const [deviceId, deviceData] of this.devices.entries()) {
        ...
        if (deviceData.recentTransactions && deviceData.recentTransactions.length > 0) {
          ...
          allTransactions = allTransactions.concat(txsWithDevice);
          // Gabungkan
        }
      }
    ...
    try {
      ...
      try {
        ...
        const options = {
          ...
          port: backendConfig.port,
          // Port backend
          path: '/api/nfc-cards/list?limit=1000',
