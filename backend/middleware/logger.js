...
const requestLogger = (req, res, next) => { // requestLogger: middleware Express; menerima (req, res, next) — req=request, r...
  ...
  const start = Date.now(); // Date.now() mengembalikan timestamp milidetik saat ini; digunakan untuk cap wa...
  ...
  res.on('finish', () => { // res.on('finish') mendaftarkan event listener; dipanggil otomatis setelah resp...
    ...
    // Contoh: GET /api/users/me - 200 - 45ms - ::1
    console.log(`${method} ${url} - ${statusCode} - ${duration}ms - ${ip}`); // console.log mencetak pesan debug ke terminal; membantu melacak alur dan nilai...
...
module.exports = { requestLogger }; // module.exports mengekspor objek/fungsi dari file ini agar bisa digunakan file...
