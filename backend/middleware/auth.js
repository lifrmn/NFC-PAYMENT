...
const jwt = require('jsonwebtoken'); // const membuat variabel tetap; require digunakan Node.js untuk memanggil modul...
...
const { PrismaClient } = require('@prisma/client'); // const membuat variabel tetap; { PrismaClient } menggunakan destructuring untu...
...
const authenticateToken = async (req, res, next) => { // const membuat variabel tetap; async membuat function bisa menggunakan await; ...
  ...
  try { // try menjalankan kode yang berpotensi error: jwt.verify, database query
    ...
    const authHeader = req.headers['authorization']; // const membuat variabel tetap; req.headers['authorization'] membaca header Aut...
    ...
    const token = authHeader && authHeader.split(' ')[1]; // const membuat variabel tetap; authHeader && ... adalah short-circuit evaluati...
    ...
    const appKey = req.headers['x-app-key']; // const membuat variabel tetap; req.headers['x-app-key'] membaca custom header ...
    // const membuat variabel tetap; req.headers['x-app-key'] membaca custom header x-app-key yang dikirim mobile app
    const appSecret = process.env.APP_SECRET || 'NFC2025SecureApp'; // const membuat variabel tetap; process.env.APP_SECRET membaca dari .env; || fa...
    // const membuat variabel tetap; process.env.APP_SECRET membaca dari .env; || fallback ke nilai default jika tidak ada
    if (appKey === appSecret) { // if mengecek apakah app key cocok dengan secret yang ditentukan; === adalah pe...
      ...
      return next(); // return menghentikan eksekusi middleware ini; next() memanggil middleware atau...
    ...
    if (!token) { // if (!...) validasi bahwa nilai tidak kosong/null sebelum melanjutkan operasi
      ...
      // Log untuk debug
      return res.status(401).json({ error: 'Access token required' }); // return menghentikan; res.status(401) mengatur HTTP status 401 Unauthorized; ....
    ...
    const jwtSecret = process.env.JWT_SECRET || 'nfc-payment-jwt-secret-2025-ultra-secure-key'; // const membuat variabel tetap; process.env.JWT_SECRET membaca secret dari .env...
    ...
    try { // try dalam try — untuk menangkap error JWT secara spesifik
      ...
      decoded = jwt.verify(token, jwtSecret); // jwt.verify memvalidasi token: cek signature menggunakan jwtSecret dan cek apa...
      ...
      console.error('❌ JWT verification failed:', jwtError.message); // console.error mencetak pesan error ke terminal dengan tanda merah; untuk debu...
      // console.error mencetak pesan error ke terminal dengan tanda merah; untuk debugging masalah
      
      if (jwtError.name === 'TokenExpiredError') { // if mengecek tipe error; .name adalah property dari Error object; TokenExpired...
        // if mengecek tipe error; .name adalah property dari Error object; TokenExpiredError terjadi ketika token melewati waktu exp
        return res.status(401).json({ // return menghentikan eksekusi dan mengirim 401 Unauthorized karena token sudah...
          // return menghentikan eksekusi dan mengirim 401 Unauthorized karena token sudah kadaluarsa
          error: 'Token expired', // field error: kode error singkat yang bisa dibaca oleh frontend untuk menampil...
      ...
      } else if (jwtError.name === 'JsonWebTokenError') { // else if kondisi lanjutan; JsonWebTokenError terjadi ketika format token salah...
        ...
        userSessions: { // include relasi userSessions dari tabel UserSession
          ...
          where: { // kondisi untuk filter session yang valid
            ...
            expiresAt: { // kondisi pada field expiresAt
              ...
              gt: new Date() // gt = greater than; new Date() adalah waktu sekarang; artinya expiresAt > seka...
    ...
    if (!user) { // if mengecek apakah user null — user tidak ditemukan di database (mungkin suda...
      ...
      // Log untuk debug
      return res.status(401).json({ // return menghentikan eksekusi dan mengirim 401 Unauthorized karena user tidak ...
    ...
    if (user.userSessions.length === 0) { // if mengecek apakah array userSessions kosong; .length mengambil panjang array...
    ...
    // Log sukses
    req.user = user; // req.user = user menyimpan data user ke objek request agar bisa diakses oleh r...
    ...
    req.token = token; // req.token menyimpan token ke objek request agar route handler bisa akses toke...
    // req.token menyimpan token ke objek request agar route handler bisa akses token jika diperlukan
    next(); // next() memanggil middleware atau route handler berikutnya — request dilanjutk...
...
const authenticateAdmin = (req, res, next) => { // middleware authenticateAdmin: memverifikasi request berasal dari admin yang s...
  ...
  const adminPassword = req.headers['x-admin-password'] || req.body.adminPassword; // req.headers membaca header HTTP; || membaca dari body jika header tidak ada; ...
  ...
  if (appKey !== appSecret) { // memeriksa apakah x-app-key yang dikirim cocok dengan APP_SECRET; tidak cocok ...
  ...
  next(); // next() memanggil middleware berikutnya dalam chain; tanpa next() request akan...
...
const authenticateDevice = (req, res, next) => { // middleware authenticateDevice: memverifikasi request sinkronisasi perangkat b...
  ...
  const appSecret = process.env.APP_SECRET || 'NFC2025SecureApp'; // ambil APP_SECRET dari environment variable; || menggunakan default jika tidak...
  // ambil APP_SECRET dari environment variable; || menggunakan default jika tidak ada
  
  // STEP 4: Validasi app key
  if (appKey !== appSecret) { // memeriksa apakah x-app-key cocok dengan secret; memastikan request dari app A...
...
module.exports = { // module.exports adalah cara CommonJS (Node.js) untuk mengekspor nilai dari fil...
  ...
  authenticateToken, // shorthand ES6: sama dengan authenticateToken: authenticateToken — digunakan u...
  // shorthand ES6: sama dengan authenticateToken: authenticateToken — digunakan untuk proteksi endpoint yang perlu JWT token
  authenticateAdmin, // shorthand ES6: sama dengan authenticateAdmin: authenticateAdmin — digunakan u...
  // shorthand ES6: sama dengan authenticateAdmin: authenticateAdmin — digunakan untuk proteksi endpoint admin
  authenticateDevice // shorthand ES6: sama dengan authenticateDevice: authenticateDevice — digunakan...
