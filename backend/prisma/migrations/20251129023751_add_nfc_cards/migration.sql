...
CREATE TABLE "nfc_cards" (
    ...
    "cardStatus" TEXT NOT NULL DEFAULT 'ACTIVE', -- status kartu: ACTIVE, BLOCKED, LOST, EXPIRED; default ACTIVE saat pertama didaftarkan
    "balance" REAL NOT NULL DEFAULT 0, -- saldo kartu dalam Rupiah; REAL = angka desimal; default 0 saat didaftarkan
    "lastUsed" DATETIME, -- waktu terakhir kartu digunakan untuk transaksi; nullable karena kartu baru belum pernah dipakai
    ...
    CONSTRAINT "nfc_cards_userId_fkey" FOREIGN KEY ("userId") REFERENCES "users" ("id") ON DELETE SET NULL ON UPDATE CASCADE -- foreign key constraint: jika user dihapus, userId di kartu di-set NULL (tidak ikut terhapus)
);
