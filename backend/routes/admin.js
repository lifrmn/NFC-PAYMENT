...
const { body, validationResult } = require('express-validator'); // const dengan destructuring; body adalah fungsi pembuat aturan validasi input;...
...
router.get('/dashboard', async (req, res) => { // router.get mendaftarkan endpoint HTTP GET; '/dashboard' adalah path relatif e...
  ...
  try { // try memulai blok percobaan; jika ada error di dalamnya, eksekusi loncat ke bl...
    ...
    const [ // const membuat variabel tetap; array destructuring untuk mengambil hasil masin...
    ...
    ] = await Promise.all([ // await menunggu semua Promise selesai; Promise.all menerima array Promise dan ...
    // await menunggu semua Promise selesai; Promise.all menerima array Promise dan menjalankan SEMUA secara paralel — lebih efisien dari await satu per satu
      ...
      prisma.user.count({ where: { isActive: true } }), // prisma.user.count() adalah setara SQL SELECT COUNT(*) FROM users WHERE isActi...
      ...
      prisma.device.count({ // prisma.device.count() dengan kondisi where: menghitung device yang aktif (onl...
        ...
        where: { // where: { } menentukan kondisi filter query; setara WHERE di SQL; hanya record...
          ...
          lastSeen: { // lastSeen: timestamp terakhir device melakukan sync; digunakan untuk menentuka...
            // lastSeen: timestamp terakhir device melakukan sync; digunakan untuk menentukan apakah device masih 'online'
            gte: new Date(Date.now() - 300000) // gte berarti greater than or equal (>=); Date.now() mengembalikan timestamp se...
      ...
      prisma.user.aggregate({ // prisma.user.aggregate(): menghitung agregat pada tabel user; setara SELECT SU...
        // prisma.user.aggregate(): menghitung agregat pada tabel user; setara SELECT SUM(balance) WHERE isActive=true
        _sum: { balance: true }, // _sum adalah operasi agregasi Prisma — setara SQL SUM(balance); true berarti f...
      ...
      prisma.fraudAlert.count({ // prisma.fraudAlert.count() dengan filter waktu: menghitung fraud alert dalam 2...
        // prisma.fraudAlert.count() dengan filter waktu: menghitung fraud alert dalam 24 jam terakhir untuk monitoring harian
        where: { // where: { } menentukan kondisi filter query; setara WHERE di SQL; hanya record...
          // where: { } menentukan kondisi filter query; setara WHERE di SQL; hanya record yang memenuhi kondisi yang dikembalikan
          createdAt: { // createdAt: field timestamp pembuatan record; difilter untuk mendapatkan alert...
      ...
      prisma.transaction.findMany({ // prisma.transaction.findMany(): mengambil 10 transaksi terbaru dengan JOIN ke ...
      ...
      prisma.fraudAlert.findMany({ // prisma.fraudAlert.findMany(): mengambil 5 fraud alert terbaru dengan data use...
    ...
    // Log error
    res.status(500).json({ error: 'Gagal mendapatkan statistik dashboard' }); // mengirim response error 500 Internal Server Error jika terjadi error tak terd...
...
router.post('/balance-update', [ // router.post() mendaftarkan endpoint HTTP POST; dipanggil saat ada request POS...
// router.post() mendaftarkan endpoint HTTP POST; dipanggil saat ada request POST ke URL tersebut
  body('deviceId').notEmpty().withMessage('ID Perangkat diperlukan'), // validasi: deviceId tidak boleh kosong; withMessage() menentukan pesan error j...
  ...
  body('adminPassword').notEmpty().withMessage('Password admin diperlukan') // validasi: adminPassword wajib ada untuk memastikan hanya admin yang bisa ubah...
  ...
  try { // try: membungkus operasi yang berisiko error; jika terjadi error akan ditangka...
    ...
    const errors = validationResult(req); // validationResult() mengumpulkan semua hasil validasi dari middleware body() y...
    ...
    if (!errors.isEmpty()) { // if (!...) validasi bahwa nilai tidak kosong/null sebelum melanjutkan operasi
      ...
      return res.status(400).json({ errors: errors.array() }); // return + res.status: menghentikan eksekusi dan langsung mengirim response err...
      // return + res.status: menghentikan eksekusi dan langsung mengirim response error 400 ke client
    }

    const { deviceId, amount, adminPassword, reason } = req.body; // destructuring req.body: mengambil 4 field dari body request untuk proses upda...
    ...
    // Validasi jumlah
    if (amount <= 0) { // validasi tambahan: amount harus lebih dari 0; meski ada validasi body(), ini ...
      // validasi tambahan: amount harus lebih dari 0; meski ada validasi body(), ini sebagai lapisan keamanan tambahan
      return res.status(400).json({ error: 'Jumlah harus positif' }); // return + res.status: menghentikan eksekusi dan langsung mengirim response err...
      // return + res.status: menghentikan eksekusi dan langsung mengirim response error 400 ke client
    }

    if (amount > 10000000) { // Maksimal 10 juta
      // Maksimal 10 juta
      return res.status(400).json({ error: 'Jumlah melebihi batas maksimum' }); // return + res.status: menghentikan eksekusi dan langsung mengirim response err...
      // return + res.status: menghentikan eksekusi dan langsung mengirim response error 400 ke client
    }

    // Cari perangkat dan pengguna
    const device = await prisma.device.findUnique({ // prisma.device.findUnique(): mencari satu device unik berdasarkan deviceId unt...
    ...
    const users = await prisma.user.findMany({ // prisma.user.findMany(): mengambil semua user dari database untuk ditampilkan ...
    ...
    // prisma.user.findMany(): mengambil semua user yang terdaftar pada device tertentu dan masih aktif
      where: { deviceId, isActive: true } // filter: hanya user dengan deviceId tersebut AND isActive=true; shorthand ES6 ...
      // filter: hanya user dengan deviceId tersebut AND isActive=true; shorthand ES6 untuk { deviceId: deviceId }
    });

    if (users.length === 0) { // memeriksa apakah array kosong — jika kosong tidak ada data yang perlu ditampi...
      // memeriksa apakah array kosong — jika kosong tidak ada data yang perlu ditampilkan
      return res.status(404).json({ error: 'Tidak ada pengguna aktif untuk perangkat ini' }); // return + res.status(404): menghentikan eksekusi dan mengirim 404 Not Found ke...
      // return + res.status(404): menghentikan eksekusi dan mengirim 404 Not Found ke client
    }

    // Perbarui saldo untuk semua pengguna di perangkat
    const updatedUsers = await Promise.all( // Promise.all(): menjalankan update saldo semua user secara paralel; lebih efis...
    // Promise.all(): menjalankan update saldo semua user secara paralel; lebih efisien dari loop serial
      users.map(async (user) => { // .map() mengubah setiap user menjadi Promise update; async karena setiap updat...
        // .map() mengubah setiap user menjadi Promise update; async karena setiap update ke database bersifat asynchronous
        const updatedUser = await prisma.user.update({ // prisma.user.update(): memperbarui saldo satu user; await karena operasi datab...
          ...
          data: { balance: { increment: amount } } // increment: menambahkan amount ke saldo existing; lebih aman dari set langsung...
          // increment: menambahkan amount ke saldo existing; lebih aman dari set langsung (menghindari race condition)
        });

        return updatedUser; // mengembalikan objek user yang telah diperbarui; dikumpulkan Promise.all() men...
    ...
    await prisma.adminLog.create({ // await prisma.adminLog.create(): mencatat aksi admin ke tabel AdminLog untuk a...
      ...
      data: { // data: { } berisi field yang akan diisi saat create atau diperbarui saat updat...
        ...
        action: 'BALANCE_UPDATE', // label aksi untuk audit trail; BALANCE_UPDATE menandakan admin mengubah saldo ...
        ...
        details: JSON.stringify({ // JSON.stringify() mengubah objek JavaScript menjadi string JSON; untuk logging...
          ...
          userIds: users.map(u => u.id), // array ID semua user yang diperbarui; untuk keperluan investigasi jika ada mas...
        ...
        ipAddress: req.ip, // req.ip: alamat IP admin yang memperbarui saldo; dicatat untuk audit trail kea...
    ...
    await prisma.device.update({ // prisma.device.update(): memperbarui total saldo device setelah saldo semua us...
      ...
      data: { // data: { } berisi field yang akan diisi saat create atau diperbarui saat updat...
        // data: { } berisi field yang akan diisi saat create atau diperbarui saat update; setara VALUES di INSERT atau SET di UPDATE
        totalBalance: { // totalBalance: field agregat di tabel device yang merupakan total saldo semua ...
    ...
    console.log(`💰 Admin menambahkan Rp ${amount.toLocaleString('id-ID')} ke perangkat ${deviceId.substring(0, 8)}... untuk ${users.length} pengguna`); // console.log mencetak pesan debug ke terminal; membantu melacak alur dan nilai...
    // console.log mencetak pesan debug ke terminal; membantu melacak alur dan nilai variabel

    // Kirim pembaruan real-time
    if (req.io) { // memeriksa apakah Socket.IO (req.io) tersedia; jika ada kirim notifikasi real-...
      // memeriksa apakah Socket.IO (req.io) tersedia; jika ada kirim notifikasi real-time ke admin dashboard yang terhubung
      req.io.to('admin-room').emit('balance-bulk-update', { // Socket.IO: mengirim event 'balance-bulk-update' ke admin dashboard untuk upda...
        // Socket.IO: mengirim event 'balance-bulk-update' ke admin dashboard untuk update tampilan saldo secara real-time
        deviceId, // deviceId perangkat yang saldo user-nya diperbarui; dikirim ke dashboard untuk...
        ...
        updatedUsers // array data user yang diperbarui; dashboard memperbarui tampilan saldo masing-...
        // array data user yang diperbarui; dashboard memperbarui tampilan saldo masing-masing user
      });

      // Notifikasi perangkat tertentu
      req.io.to(`device-${deviceId}`).emit('balance-updated', { // Socket.IO: mengirim event ke room khusus device ini; device dapat update sald...
        ...
        users: updatedUsers.map(u => ({ id: u.id, balance: u.balance })) // .map() membuat array ringkas dengan hanya id dan balance; dikirim ke device u...
    ...
    res.json({ // res.json(): mengirim respons HTTP dengan Content-Type application/json; mengo...
      ...
      message: `Saldo diperbarui untuk ${users.length} pengguna di perangkat ${deviceId}`, // template literal: pesan konfirmasi dinamis dengan jumlah user dan deviceId ya...
      // template literal: pesan konfirmasi dinamis dengan jumlah user dan deviceId yang terpengaruh
      details: { // objek details berisi informasi teknis operasi untuk referensi dan audit lebih...
    ...
    // catch (error): menangkap semua error dari blok try untuk penanganan yang aman
    console.error('❌ Kesalahan pembaruan saldo:', error); // console.error mencetak pesan error ke terminal dengan tanda merah; untuk debu...
...
// Dapatkan log admin
router.get('/logs', async (req, res) => { // router.get() mendaftarkan endpoint HTTP GET; dipanggil saat ada request GET k...
  // router.get() mendaftarkan endpoint HTTP GET; dipanggil saat ada request GET ke URL tersebut
  try { // try: membungkus operasi yang berisiko error; jika terjadi error akan ditangka...
    // try: membungkus operasi yang berisiko error; jika terjadi error akan ditangkap oleh catch
    const { limit = 50, offset = 0, action } = req.query; // destructuring req.query dengan default: limit=50 untuk pagination, offset=0 u...
    ...
    if (action) whereClause.action = action; // kondisional: hanya tambahkan filter action ke whereClause jika parameter acti...
    // kondisional: hanya tambahkan filter action ke whereClause jika parameter action dikirim; filter dinamis

    const logs = await prisma.adminLog.findMany({ // prisma.adminLog.findMany(): mengambil log aktivitas admin dari database untuk...
      ...
      take: parseInt(limit), // parseInt() mengubah string menjadi bilangan bulat; digunakan untuk ID atau ju...
      // parseInt() mengubah string menjadi bilangan bulat; digunakan untuk ID atau jumlah item
      skip: parseInt(offset) // parseInt() mengubah string menjadi bilangan bulat; digunakan untuk ID atau ju...
      // parseInt() mengubah string menjadi bilangan bulat; digunakan untuk ID atau jumlah item
    });

    res.json(logs); // mengirim array log admin sebagai JSON response ke client; tanpa transformasi ...
    // mengirim array log admin sebagai JSON response ke client; tanpa transformasi tambahan
  } catch (error) { // catch (error): menangkap semua error dari blok try untuk penanganan yang aman
    // catch (error): menangkap semua error dari blok try untuk penanganan yang aman
    console.error('❌ Kesalahan mendapatkan log admin:', error); // console.error mencetak pesan error ke terminal dengan tanda merah; untuk debu...
    // console.error mencetak pesan error ke terminal dengan tanda merah; untuk debugging masalah
    res.status(500).json({ error: 'Gagal mendapatkan log admin' }); // mengirim response error 500 Internal Server Error jika terjadi error tak terd...
    // mengirim response error 500 Internal Server Error jika terjadi error tak terduga di server
  }
});

// Manajemen pengaturan sistem
router.get('/settings', async (req, res) => { // router.get() mendaftarkan endpoint HTTP GET; dipanggil saat ada request GET k...
  // router.get() mendaftarkan endpoint HTTP GET; dipanggil saat ada request GET ke URL tersebut
  try { // try: membungkus operasi yang berisiko error; jika terjadi error akan ditangka...
    // try: membungkus operasi yang berisiko error; jika terjadi error akan ditangkap oleh catch
    const settings = await prisma.systemSettings.findMany(); // prisma.systemSettings.findMany(): mengambil semua pengaturan sistem dari data...
    // prisma.systemSettings.findMany(): mengambil semua pengaturan sistem dari database; disimpan sebagai key-value pairs
    
    const settingsObject = settings.reduce((acc, setting) => { // .reduce() mengubah array [{key, value, type}] menjadi object {key: parsedValu...
      // .reduce() mengubah array [{key, value, type}] menjadi object {key: parsedValue}; lebih mudah dikonsumsi frontend
      let value = setting.value; // let (bukan const) karena value akan dikonversi tipenya di switch di bawah ber...
      ...
      switch (setting.type) { // switch: memeriksa tipe data setting; string, number, boolean, json perlu pena...
        // switch: memeriksa tipe data setting; string, number, boolean, json perlu penanganan konversi yang berbeda
        case 'number': // jika setting bertipe number: konversi dari string (disimpan di DB) ke tipe Nu...
          // jika setting bertipe number: konversi dari string (disimpan di DB) ke tipe Number JavaScript
          value = parseFloat(setting.value); // parseFloat() mengubah string menjadi angka desimal; digunakan untuk nilai Z-S...
        ...
        case 'boolean': // jika setting bertipe boolean: konversi string 'true'/'false' ke tipe boolean ...
          // jika setting bertipe boolean: konversi string 'true'/'false' ke tipe boolean JavaScript
          value = setting.value === 'true'; // strict comparison: === 'true' menghasilkan true jika string sama persis 'true...
        ...
        case 'json': // jika setting bertipe json: parse string JSON menjadi objek/array JavaScript
          ...
          try { // try: membungkus operasi yang berisiko error; jika terjadi error akan ditangka...
            // try: membungkus operasi yang berisiko error; jika terjadi error akan ditangkap oleh catch
            value = JSON.parse(setting.value); // JSON.parse() mengubah string JSON menjadi objek JavaScript; untuk membaca dat...
        ...
        default: // default: tipe lainnya (string, dll) tidak perlu konversi; value tetap sebagai...
      ...
      acc[setting.key] = value; // dynamic key assignment: menggunakan setting.key sebagai nama properti di obje...
    ...
    res.json(settingsObject); // mengirim objek pengaturan sistem sebagai JSON; frontend dapat langsung akses ...
    // mengirim objek pengaturan sistem sebagai JSON; frontend dapat langsung akses nilai dengan settingsObject.key
  } catch (error) { // catch (error): menangkap semua error dari blok try untuk penanganan yang aman
    // catch (error): menangkap semua error dari blok try untuk penanganan yang aman
    console.error('❌ Kesalahan mendapatkan pengaturan:', error); // console.error mencetak pesan error ke terminal dengan tanda merah; untuk debu...
    // console.error mencetak pesan error ke terminal dengan tanda merah; untuk debugging masalah
    res.status(500).json({ error: 'Gagal mendapatkan pengaturan' }); // mengirim response error 500 Internal Server Error jika terjadi error tak terd...
    // mengirim response error 500 Internal Server Error jika terjadi error tak terduga di server
  }
});

// Perbarui pengaturan sistem
router.put('/settings/:key', [ // router.put() mendaftarkan endpoint HTTP PUT; untuk memperbarui data yang suda...
// router.put() mendaftarkan endpoint HTTP PUT; untuk memperbarui data yang sudah ada
  ...
  try { // try: membungkus operasi yang berisiko error; jika terjadi error akan ditangka...
    // try: membungkus operasi yang berisiko error; jika terjadi error akan ditangkap oleh catch
    const errors = validationResult(req); // validationResult() mengumpulkan semua error validasi dari middleware body() d...
    ...
    const { key } = req.params; // destructuring req.params: mengambil parameter key dari URL dinamis /settings/...
    // destructuring req.params: mengambil parameter key dari URL dinamis /settings/:key
    const { value, type = 'string', adminPassword } = req.body; // destructuring req.body: value=nilai baru, type dengan default 'string', admin...
    ...
    const setting = await prisma.systemSettings.upsert({ // prisma.systemSettings.upsert(): buat setting baru atau update jika sudah ada;...
      ...
      update: { value: String(value), type }, // String() mengkonversi nilai ke tipe string; digunakan saat perlu teks dari ni...
      // String() mengkonversi nilai ke tipe string; digunakan saat perlu teks dari nilai non-string
      create: { key, value: String(value), type } // String() mengkonversi nilai ke tipe string; digunakan saat perlu teks dari ni...
      // String() mengkonversi nilai ke tipe string; digunakan saat perlu teks dari nilai non-string
    });
    ...
    if (req.io) { // memeriksa apakah Socket.IO (req.io) tersedia; jika ada kirim notifikasi real-...
      // memeriksa apakah Socket.IO (req.io) tersedia; jika ada kirim notifikasi real-time ke admin dashboard yang terhubung
      req.io.to('admin-room').emit('setting-updated', { key, value, type }); // Socket.IO: mengirim event ke admin room agar pengaturan langsung diperbarui d...
    ...
    res.json({ // res.json(): mengirim respons HTTP dengan Content-Type application/json; mengo...
      ...
      setting // objek setting yang baru disimpan; dikembalikan sebagai konfirmasi data yang t...
      // objek setting yang baru disimpan; dikembalikan sebagai konfirmasi data yang tersimpan
    });

  } catch (error) { // catch (error): menangkap semua error dari blok try untuk penanganan yang aman
    // catch (error): menangkap semua error dari blok try untuk penanganan yang aman
    console.error('❌ Kesalahan memperbarui pengaturan:', error); // console.error mencetak pesan error ke terminal dengan tanda merah; untuk debu...
    // console.error mencetak pesan error ke terminal dengan tanda merah; untuk debugging masalah
    res.status(500).json({ error: 'Gagal memperbarui pengaturan' }); // mengirim response error 500 Internal Server Error jika terjadi error tak terd...
    // mengirim response error 500 Internal Server Error jika terjadi error tak terduga di server
  }
});

// Bersihkan perangkat tidak aktif
router.post('/cleanup-devices', async (req, res) => { // router.post() mendaftarkan endpoint HTTP POST; dipanggil saat ada request POS...
  // router.post() mendaftarkan endpoint HTTP POST; dipanggil saat ada request POST ke URL tersebut
  try { // try: membungkus operasi yang berisiko error; jika terjadi error akan ditangka...
    // try: membungkus operasi yang berisiko error; jika terjadi error akan ditangkap oleh catch
    const { adminPassword } = req.body; // destructuring req.body: mengambil adminPassword dari body request untuk verif...
    ...
    const cutoffTime = new Date(Date.now() - 24 * 60 * 60 * 1000); // Date.now() mengembalikan timestamp milidetik saat ini; digunakan untuk cap wa...
    // Date.now() mengembalikan timestamp milidetik saat ini; digunakan untuk cap waktu operasi
    
    const deletedDevices = await prisma.device.deleteMany({ // prisma.device.deleteMany(): menghapus banyak device sekaligus yang tidak akti...
      // prisma.device.deleteMany(): menghapus banyak device sekaligus yang tidak aktif; setara DELETE WHERE di SQL
      where: { // where: { } menentukan kondisi filter query; setara WHERE di SQL; hanya record...
        // where: { } menentukan kondisi filter query; setara WHERE di SQL; hanya record yang memenuhi kondisi yang dikembalikan
        lastSeen: { // lastSeen: timestamp terakhir device sync; field yang menjadi kriteria penghap...
          // lastSeen: timestamp terakhir device sync; field yang menjadi kriteria penghapusan device tidak aktif
          lt: cutoffTime // lt = less than (<); hapus device yang lastSeen-nya SEBELUM cutoffTime (tidak ...
    ...
    // Catat aksi admin
    await prisma.adminLog.create({ // await prisma.adminLog.create(): mencatat aksi cleanup device ke tabel AdminLo...
    ...
    res.json({ // res.json(): mengirim respons HTTP dengan Content-Type application/json; mengo...
      // res.json(): mengirim respons HTTP dengan Content-Type application/json; mengonversi objek JavaScript ke JSON string otomatis
      message: `Membersihkan ${deletedDevices.count} perangkat tidak aktif`, // template literal: pesan konfirmasi dinamis berapa device tidak aktif yang ber...
    ...
    // catch (error): menangkap semua error dari blok try untuk penanganan yang aman
    console.error('Cleanup devices error:', error); // console.error mencetak pesan error ke terminal dengan tanda merah; untuk debu...
    // console.error mencetak pesan error ke terminal dengan tanda merah; untuk debugging masalah
...
router.get('/users', async (req, res) => { // router.get() mendaftarkan endpoint HTTP GET; dipanggil saat ada request GET k...
  // router.get() mendaftarkan endpoint HTTP GET; dipanggil saat ada request GET ke URL tersebut
  try { // try: membungkus operasi yang berisiko error; jika terjadi error akan ditangka...
    // try: membungkus operasi yang berisiko error; jika terjadi error akan ditangkap oleh catch
    console.log('📋 Admin request: Get all users (bypass auth)'); // console.log mencetak pesan debug ke terminal; membantu melacak alur dan nilai...
    // console.log mencetak pesan debug ke terminal; membantu melacak alur dan nilai variabel
    
    // STEP 1: Query semua user dari database
    const users = await prisma.user.findMany({ // prisma.user.findMany(): mengambil semua user dari database untuk halaman mana...
      ...
      orderBy: { // ORDER BY createdAt DESC (user terbaru di atas)
        ...
        createdAt: 'desc' // urutkan berdasarkan tanggal dibuat terbaru; admin biasanya ingin lihat user t...
    ...
    console.log(`✅ Found ${users.length} users in database`); // console.log mencetak pesan debug ke terminal; membantu melacak alur dan nilai...
    // console.log mencetak pesan debug ke terminal; membantu melacak alur dan nilai variabel
    ...
    // catch (error): menangkap semua error dari blok try untuk penanganan yang aman
    console.error('❌ Get admin users error:', error); // console.error mencetak pesan error ke terminal dengan tanda merah; untuk debu...
    // console.error mencetak pesan error ke terminal dengan tanda merah; untuk debugging masalah
...
// Body: { amount: 50000 }
router.post('/bulk-topup', async (req, res) => { // router.post() mendaftarkan endpoint HTTP POST; dipanggil saat ada request POS...
  // router.post() mendaftarkan endpoint HTTP POST; dipanggil saat ada request POST ke URL tersebut
  try { // try: membungkus operasi yang berisiko error; jika terjadi error akan ditangka...
    // try: membungkus operasi yang berisiko error; jika terjadi error akan ditangkap oleh catch
    // STEP 1: Ambil amount dari request body
    const { amount } = req.body; // destructuring req.body: mengambil amount (jumlah saldo) yang akan ditambahkan...
    // destructuring req.body: mengambil amount (jumlah saldo) yang akan ditambahkan ke semua user aktif
    
    console.log(`💰 Admin bulk topup request: ${amount} to all users`); // console.log mencetak pesan debug ke terminal; membantu melacak alur dan nilai...
    // console.log mencetak pesan debug ke terminal; membantu melacak alur dan nilai variabel
    ...
    const updateResult = await prisma.user.updateMany({ // prisma.user.updateMany(): memperbarui banyak record sekaligus; setara UPDATE ...
    ...
    console.log(`✅ Bulk topup success: ${updateResult.count} users updated with ${amount}`); // console.log mencetak pesan debug ke terminal; membantu melacak alur dan nilai...
    // console.log mencetak pesan debug ke terminal; membantu melacak alur dan nilai variabel
    ...
    res.json({ // res.json(): mengirim respons HTTP dengan Content-Type application/json; mengo...
      ...
      success: true, // success: true menandakan operasi berhasil; frontend memeriksa field ini untuk...
      // success: true menandakan operasi berhasil; frontend memeriksa field ini untuk menentukan apakah perlu tampilkan sukses atau error
      message: `Successfully topped up ${updateResult.count} users`, // template literal: pesan konfirmasi dinamis dengan jumlah user yang berhasil d...
...
router.post('/reset-balance', async (req, res) => { // router.post() mendaftarkan endpoint HTTP POST; dipanggil saat ada request POS...
  // router.post() mendaftarkan endpoint HTTP POST; dipanggil saat ada request POST ke URL tersebut
  try { // try: membungkus operasi yang berisiko error; jika terjadi error akan ditangka...
    // try: membungkus operasi yang berisiko error; jika terjadi error akan ditangkap oleh catch
    // STEP 1: Ambil data dari request body
    const { userId, newBalance, password } = req.body; // destructuring req.body: userId=target user, newBalance=saldo baru, password=v...
    // destructuring req.body: userId=target user, newBalance=saldo baru, password=verifikasi admin
    
    // STEP 2: Validasi admin password
    if (password !== (process.env.ADMIN_PASSWORD || 'admin123')) { // verifikasi password admin; !== kecocokan string persis; fallback 'admin123' j...
    ...
    console.log(`💰 Reset balance: ${user.username} -> Rp ${parseInt(newBalance).toLocaleString('id-ID')}`); // console.log mencetak pesan debug ke terminal; membantu melacak alur dan nilai...
    // console.log mencetak pesan debug ke terminal; membantu melacak alur dan nilai variabel
...
router.post('/block-user', async (req, res) => { // router.post() mendaftarkan endpoint HTTP POST; dipanggil saat ada request POS...
  // router.post() mendaftarkan endpoint HTTP POST; dipanggil saat ada request POST ke URL tersebut
  try { // try: membungkus operasi yang berisiko error; jika terjadi error akan ditangka...
    // try: membungkus operasi yang berisiko error; jika terjadi error akan ditangkap oleh catch
    // STEP 1: Ambil data dari request body
    const { userId, password } = req.body; // destructuring req.body: userId=user yang akan diblokir, password=verifikasi a...
    ...
    // STEP 4: Log action ke console
    console.log(`🚫 User blocked: ${userId} (${user.username})`); // console.log mencetak pesan debug ke terminal; membantu melacak alur dan nilai...
    // console.log mencetak pesan debug ke terminal; membantu melacak alur dan nilai variabel
...
// Body: { userId: 123, password: 'admin123' }
router.post('/unblock-user', async (req, res) => { // router.post() mendaftarkan endpoint HTTP POST; dipanggil saat ada request POS...
  // router.post() mendaftarkan endpoint HTTP POST; dipanggil saat ada request POST ke URL tersebut
  try { // try: membungkus operasi yang berisiko error; jika terjadi error akan ditangka...
    // try: membungkus operasi yang berisiko error; jika terjadi error akan ditangkap oleh catch
    // STEP 1: Ambil data dari request body
    const { userId, password } = req.body; // destructuring req.body: userId=user yang akan di-unblock, password=verifikasi...
    ...
    // STEP 4: Log action ke console
    console.log(`✅ User unblocked: ${userId} (${user.username})`); // console.log mencetak pesan debug ke terminal; membantu melacak alur dan nilai...
    // console.log mencetak pesan debug ke terminal; membantu melacak alur dan nilai variabel
...
router.post('/clear-fraud-alerts', async (req, res) => { // router.post() mendaftarkan endpoint HTTP POST; dipanggil saat ada request POS...
  // router.post() mendaftarkan endpoint HTTP POST; dipanggil saat ada request POST ke URL tersebut
  try { // try: membungkus operasi yang berisiko error; jika terjadi error akan ditangka...
    // try: membungkus operasi yang berisiko error; jika terjadi error akan ditangkap oleh catch
    // STEP 1: Count fraud alerts sebelum deletion (untuk info berapa yang dihapus)
    const alertCount = await prisma.fraudAlert.count(); // prisma.fraudAlert.count(): hitung jumlah alert sebelum dihapus; digunakan dal...
    ...
    await prisma.fraudAlert.deleteMany({}); // prisma.fraudAlert.deleteMany() dengan objek kosong: hapus SEMUA record dari t...
    ...
    console.log(`🗑️ Cleared ${alertCount} fraud alerts`); // console.log mencetak pesan debug ke terminal; membantu melacak alur dan nilai...
    // console.log mencetak pesan debug ke terminal; membantu melacak alur dan nilai variabel
