...
router.post('/balance-update', [ // router.post() mendaftarkan endpoint HTTP POST; dipanggil saat ada request POST ke URL tersebut
  ...
  body('adminPassword').notEmpty().withMessage('Password admin diperlukan') // validasi: adminPassword wajib ada untuk memastikan hanya admin yang bisa ubah setting // validasi: adminPassword wajib ada untuk mencegah akses tidak terotorisasi
  ...
  try { // try: membungkus operasi yang berisiko error; jika terjadi error akan ditangkap oleh catch
    ...
    const users = await prisma.user.findMany({ // prisma.user.findMany(): mengambil semua user dari database untuk ditampilkan di halaman manajemen user admin // prisma.user.findMany(): mengambil semua user yang terdaftar pada device tertentu dan masih aktif
  ...
  try { // try: membungkus operasi yang berisiko error; jika terjadi error akan ditangkap oleh catch
    ...
    const settingsObject = settings.reduce((acc, setting) => { // .reduce() mengubah array [{key, value, type}] menjadi object {key: parsedValue}; lebih mudah dikonsumsi frontend
      ...
      switch (setting.type) { // switch: memeriksa tipe data setting; string, number, boolean, json perlu penanganan konversi yang berbeda
        ...
        default: // default: tipe lainnya (string, dll) tidak perlu konversi; value tetap sebagai string // default: nilai kembalian jika tidak ada case yang cocok; nilai fallback untuk case yang tidak terdefinisi
