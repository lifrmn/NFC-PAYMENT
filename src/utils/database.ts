...
// 2. Caching: Menyimpan snapshot user untuk fallback setelah request backend gagal
// 3. Degraded Read: Cache dapat menyediakan data lama saat error non-auth
// 4. Consistency: Semua files menggunakan functions yang sama
//
// Pattern yang Digunakan:
// - Wrapper Functions: Thin wrapper around apiService methods
// - Caching Strategy: Backend-first dengan fallback snapshot AsyncStorage
// - Error Handling: Error auth diteruskan; error non-auth dapat memakai cache
...
// AsyncStorage menyimpan snapshot user dan data sesi; pembacaan user utama tetap meminta backend lebih dulu.
...
export interface Transaction {
  ...
  fraudRiskLevel?: 'NORMAL' | 'SUSPICIOUS' | 'ANOMALY' | null;
  fraudDecision?: 'ALLOW' | 'REVIEW' | 'BLOCK' | null;
  createdAt: string;
...
// getUserById memakai backend-first dan cache hanya sebagai degraded read setelah error non-auth.
//
// Caching Strategy:
// 1. Fetch backend lebih dulu agar saldo dan status akun tetap mutakhir.
// 2. Simpan response valid ke AsyncStorage untuk fallback berikutnya.
// 3. Gunakan cache hanya saat error non-auth seperti jaringan atau rate limit.
// 4. Error 401 atau akun nonaktif dilempar ulang dan tidak boleh ditutupi cache lama.
//
...
//
// CACHING FLOW:
// 1. Fetch backend dan validasi bentuk response.
// 2. Simpan user valid ke cache.
// 3. Jika error non-auth, baca cache sebagai data terakhir yang tersedia.
// 4. Jika session tidak valid atau akun nonaktif, teruskan error ke pengelola auth.
//
// USE CASE:
...
// fungsi async untuk mendapatkan data user berdasarkan ID; prioritas backend API, fallback ke cache AsyncStorage jika offline/error
  ...
  try {
    ...
    // Backend gagal non-auth (misal: 429 rate limit, network error) → gunakan cache sebagai fallback
...
// 3. Validate every transaction and related user
// 4. Propagate request or response-contract errors to the Dashboard
//
// USE CASE:
...
// getUserTransactions: fungsi async untuk mengambil riwayat transaksi user tertentu
  ...
  const isValidParty = (party: unknown): boolean => {
    const value = party as { id?: unknown; name?: unknown; username?: unknown } | null;
    return Boolean(
      value &&
  ...
  const isValidTransaction = (transaction: unknown): boolean => {
    const value = transaction as Partial<Transaction> | null;
    const validRiskLevels = new Set(['NORMAL', 'SUSPICIOUS', 'ANOMALY']);
    const validFraudDecisions = new Set(['ALLOW', 'REVIEW', 'BLOCK']);
    return Boolean(
      value &&
      Number.isSafeInteger(value.id) &&
      Number.isSafeInteger(value.senderId) &&
      Number.isSafeInteger(value.receiverId) &&
      typeof value.amount === 'number' &&
      Number.isFinite(value.amount) &&
      value.amount > 0 &&
      (value.fraudRiskScore === undefined ||
        value.fraudRiskScore === null ||
        (typeof value.fraudRiskScore === 'number' && Number.isFinite(value.fraudRiskScore))) &&
      (value.fraudRiskLevel === undefined ||
        value.fraudRiskLevel === null ||
        validRiskLevels.has(value.fraudRiskLevel)) &&
      (value.fraudDecision === undefined ||
        value.fraudDecision === null ||
        validFraudDecisions.has(value.fraudDecision)) &&
      typeof value.createdAt === 'string' &&
      Number.isFinite(Date.parse(value.createdAt)) &&
      isValidParty(value.sender) &&
      isValidParty(value.receiver)
    );
  };
  if (!Array.isArray(res) || !res.every(isValidTransaction)) {
    throw new Error('Respons histori transaksi backend tidak valid');
  }
  return res;
};
...
// 1. Backend tetap menjadi sumber saldo otoritatif.
// 2. updateUserBalance hanya mengubah snapshot lokal untuk respons UI sementara.
// 3. syncBalanceFromBackend mengambil saldo server lalu mengganti snapshot lokal.
// 4. Tidak ada antrean mutasi offline; cache tidak pernah mengubah saldo backend.
//
// ==================================================================================
...
// - Local Snapshot: Menyamakan cache dengan saldo yang sudah diketahui caller
//
// PARAMETER:
...
export const updateUserBalance = async (userId: number, newBalance: number): Promise<boolean> => {
  // updateUserBalance hanya memperbarui cache lokal; fungsi ini tidak mengirim mutasi saldo ke backend.
  try {
    // try: membungkus operasi yang berisiko error; jika terjadi error akan ditangkap oleh catch
...
export const syncBalanceFromBackend = async (userId: number): Promise<number | null> => {
  ...
  try {
    ...
    try {
      ...
      if (cachedUser) {
        // Backend sudah gagal; gunakan snapshot cache sebagai nilai lama yang masih tersedia.
        const user = JSON.parse(cachedUser);
        // JSON.parse() mengubah string JSON menjadi objek JavaScript; untuk membaca data tersimpan
...
// tidak ada koneksi database lokal yang perlu dibuka atau ditutup.
// Health check backend yang nyata dilakukan terpisah melalui apiService.healthCheck().
//
// ==================================================================================
...
// ================================================================================
// TUJUAN:
// Menyediakan initializer kompatibilitas untuk urutan startup lama.
//
// FLOW:
// 1. Tidak membuka database atau melakukan request jaringan
// 2. Mengembalikan true agar startup lanjut ke health check terpisah
//
// USE CASE:
// - App Startup: Pertahankan kontrak initializer tanpa resource lokal
//
// RETURN:
// - Promise<boolean> - true pada implementasi no-op saat ini
// ================================================================================
export const initDatabase = async (): Promise<boolean> => {
  // initDatabase adalah no-op kompatibilitas; ketersediaan backend belum diuji di fungsi ini.
  try {
    // try: membungkus operasi yang berisiko error; jika terjadi error akan ditangkap oleh catch
    console.log('🔗 Connecting to backend...');
    // console.log mencetak pesan debug ke terminal; membantu melacak alur dan nilai variabel
    
    // Tidak ada database lokal atau request jaringan; true hanya berarti initializer selesai.
    const connected = true;
    // Health check aktual dijalankan oleh tahap startup berikutnya.
    ...
    if (connected) {
      // Cabang ini selalu aktif pada implementasi no-op saat ini.
    ...
    return connected;
    // Nilai true tidak membuktikan backend online.
    ...
    // console.error mencetak pesan error ke terminal dengan tanda merah; untuk debugging masalah
    return false;
    // False hanya terjadi bila initializer lokal ini melempar error tak terduga.
  }
};
