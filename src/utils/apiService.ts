...
// - Session Failure Handling: Response 401 menghapus sesi lokal dan meminta login ulang
// - Error Handling: Menangani timeout, network error, dan response HTTP gagal
//
// Struktur Class:
...
// API_URL adalah base URL backend; autentikasi pengguna hanya memakai JWT.

const API_DISCOVERY_TIMEOUT_MS = 1800;
const DEV_FALLBACK_BASE_URLS = ['http://10.0.2.2:4000', 'http://localhost:4000', 'http://127.0.0.1:4000'];

const resolveExpoHostBaseUrl = (): string | null => {
  ...
  const hostWithPort = expoConfigHostUri || manifest2DebuggerHost || manifestDebuggerHost;
...
export class APIService {
  ...
  // JWT token dari backend, null jika belum login
  private userId: string | null = null;
  // ID user yang login, null jika belum login
  private authenticationFailureHandler: (() => void) | null = null;
  // Root app handler yang memindahkan UI ke signed-out saat sesi backend tidak valid.
  
  // Base URL backend API dari file configuration.ts
  // Semua endpoint akan digabung dengan URL ini
  private baseUrl = API_URL;
  // Contoh: 'https://xyz.ngrok-free.dev'

  private async isBaseUrlReachable(baseUrl: string, timeoutMs = API_DISCOVERY_TIMEOUT_MS): Promise<boolean> {
    try {
  ...
  private async resolveReachableBaseUrl(): Promise<string> {
    // Jangan override URL production; fallback discovery hanya untuk local development.
    if (!__DEV__) return this.baseUrl;

    const expoHostBaseUrl = resolveExpoHostBaseUrl();
    const candidates = [this.baseUrl, expoHostBaseUrl, ...DEV_FALLBACK_BASE_URLS].filter(Boolean);
    const uniqueCandidates = Array.from(new Set(candidates));
    for (const candidate of uniqueCandidates) {
      const reachable = await this.isBaseUrlReachable(candidate);
      if (reachable) {
        if (candidate !== this.baseUrl) {
          console.log(`🔁 API URL fallback: ${this.baseUrl} -> ${candidate}`);
        }
        return candidate;
      }
    }
    console.warn(`⚠️ Tidak ada API URL yang reachable. Tetap pakai: ${this.baseUrl}`);
    return this.baseUrl;
  }
  ...
  async initialize(): Promise<boolean> {
    ...
    try {
      ...
      this.baseUrl = await this.resolveReachableBaseUrl();
      // Coba fallback URL development agar app tidak tersangkut pada IP LAN yang sudah berubah.
      ...
      // Log error detail
      return false;
      // Return false untuk menandakan gagal
    }
  }
  ...
  // 4. Session Failure: Logout pada 401 atau 403 ACCOUNT_INACTIVE
  ...
  private async makeRequest(endpoint: string, options: any = {}): Promise<any> {
    ...
    const fullUrl = `${this.baseUrl}${sep}${endpoint}`;
    // Build URL lengkap
    ...
    try {
      ...
      const controller = new AbortController();
      // Buat controller untuk kontrol request
      ...
      const requestTimeoutMs = typeof options.timeoutMs === 'number' ? options.timeoutMs : 15000;
      ...
      const headers: any = {
        ...
        'Accept': 'application/json',
        // Expect respons dalam format JSON
        'ngrok-skip-browser-warning': 'true',
        // Header khusus agar Ngrok tidak redirect ke warning page
        'User-Agent': 'NFC-Payment-Mobile',
        // Identitas aplikasi untuk logging backend
      };
      ...
      const requestConfig: any = {
        ...
        signal: controller.signal,
        // Signal untuk cancel request
      };

      // STEP 4: Log request untuk memudahkan debugging
      console.log(`📱 API Call: ${requestConfig.method} ${fullUrl}`);
      // Log method dan URL
      ...
      if (!response.ok) {
        ...
        // SUBSTEP 8b: Hanya 401 yang menandakan sesi autentikasi tidak valid.
        // 403 juga digunakan untuk penolakan bisnis seperti fraud BLOCK atau kartu nonaktif.
        if (response.status === 401 && this.token) {
          // 401 saat token ada berarti sesi tidak valid; cabut sesi lokal agar user login ulang.
        ...
        // Lempar exception yang akan di-catch oleh caller
      }
      ...
      if (contentType.includes('application/json')) {
        ...
        // SUBSTEP 9b: Simpan token jika response autentikasi mengandung token baru.
        // Client tidak meminta refresh token secara otomatis; blok ini hanya menyimpan token yang diterima.
        if (result?.token) {
          // optional chaining ?.token memastikan token tersedia sebelum disimpan.
      ...
      if (error?.name === 'AbortError' && __DEV__ && !options.__retriedAfterAbort) {
        const previousBaseUrl = this.baseUrl;
        const resolvedBaseUrl = await this.resolveReachableBaseUrl();
        if (resolvedBaseUrl !== previousBaseUrl) {
          this.baseUrl = resolvedBaseUrl;
          console.warn(`🔁 Retry request setelah AbortError: ${previousBaseUrl} -> ${resolvedBaseUrl}`);
          return await this.makeRequest(endpoint, { ...options, __retriedAfterAbort: true });
        }
      }
      ...
      const isExpectedCardCheck404 = isHttpError && endpoint.includes('nfc-cards/info') && error.status === 404;
  ...
  async get(endpoint: string) {
    // async get: wrapper untuk HTTP GET request; endpoint adalah path API yang dituju
    return await this.makeRequest(endpoint, { method: 'GET' });
    // Wrapper: panggil makeRequest
  }
  ...
  async post(endpoint: string, body?: any, headers?: Record<string, string>) {
    ...
    return await this.makeRequest(endpoint, { method: 'POST', body, headers });
  ...
  async login(credentials: { username: string; password: string }) {
    ...
    const response = await this.makeRequest('/api/auth/login', {
      ...
      body: { ...credentials, deviceId },
      // Sertakan ID perangkat stabil agar sesi backend terikat ke perangkat ini.
    });
  ...
  async register(userData: { name: string; username: string; password: string }) {
    ...
    return await this.makeRequest('/api/auth/register', {
      ...
      method: 'POST',
      // method 'POST': HTTP method untuk mengirim data baru ke server; digunakan untuk create resource atau submit data
      body: { ...userData, deviceId },
      // Sertakan ID perangkat stabil untuk identitas perangkat dan sesi awal.
    });
  ...
  //
  // FLOW:
  // 1. Revoke session aktif di backend jika token tersedia
  // 2. Clear token dan userId dari memory dan persistent storage
  ...
  // ================================================================================
  async logout(revokeServerSession = true) {
    ...
    if (revokeServerSession && tokenToRevoke) {
      try {
        ...
        console.warn('Backend session could not be revoked:', error);
      }
    }
    ...
    if (cleanupResults.some(result => result.status === 'rejected')) {
      console.warn('Some local session data could not be removed');
    }
  ...
  // Endpoint memerlukan bearer token dan hanya mengizinkan akun terkait atau admin.
  //
  // USE CASE:
  ...
  async getUserById(id: number) {
    ...
    // Backend aktif mengirim objek user langsung; wrapper { user } tetap dinormalisasi defensif.
  ...
  }, idempotencyKey = `transfer-${Date.now()}-${Math.random().toString(36).slice(2, 12)}`) {
  ...
  //
  // NFC Payment Flow:
  // 1. User menempelkan kartu atau tag NFC fisik ke perangkat
  ...
  // ================================================================================
  // TUJUAN:
  // Mengirim data tag NFC ke backend untuk validasi dan pemrosesan pembayaran.
  //
  // FLOW:
  ...
  async validateNFCReceiver(nfcData: any) {
    ...
    return await this.makeRequest('/api/nfc-cards/tap', {
      // POST ke /api/nfc-cards/tap untuk validasi tap kartu NFC; backend mengembalikan info user terkait kartu
      method: 'POST',
      // method 'POST': HTTP method untuk mengirim data baru ke server; digunakan untuk create resource atau submit data
      body: { nfcData },
  ...
  async getCardInfo(cardId: string) {
    ...
    // GET ke endpoint nfc-cards/info/:cardId untuk cek apakah kartu sudah terdaftar dan info user terkait
  }
  ...
  async getAllTransactions() {
    // async getAllTransactions: mengambil semua transaksi sistem untuk laporan admin
  ...
  async registerDevice(deviceInfo: {
    ...
    return await this.makeRequest('/api/devices/register', {
      ...
      // timeoutMs opsional agar caller bisa membatasi waktu tunggu tanpa Promise.race terpisah.
    });
  }
  ...
  //
  // USE CASE:
  // - Pull saldo terbaru setelah koneksi backend tersedia
  ...
  async syncDeviceData() {
    // syncDeviceData menyediakan pull manual; tidak ada scheduler aktif yang memanggilnya secara periodik.
    ...
    // Endpoint backend yang tersedia: POST /api/devices/sync-device
    return await this.makeRequest('/api/devices/sync-device', {
      method: 'POST',
      body: { device },
    });
    // POST ke endpoint sync perangkat agar payload sesuai kontrak backend.
  }
  ...
  // ================================================================================
  async healthCheck(options: { timeoutMs?: number } = {}) {
    ...
    return await this.makeRequest('/api/health', { timeoutMs: options.timeoutMs });
  ...
  getConnectionStatus() {
    ...
    return {
      ...
      // Boolean: apakah user punya token
      userId: this.userId,
  ...
  async getDeviceId(): Promise<string> {
    ...
    try {
      ...
      if (!deviceId) {
        ...
        deviceId = `${Platform.OS}_${Date.now()}_${Math.random().toString(36).slice(2, 12)}`;
        // String() mengkonversi nilai ke tipe string; digunakan saat perlu teks dari nilai non-string
