...
//
// Purpose:
// Main NFC payment screen untuk menerima pembayaran NFC.
// Implement flow: Input amount → Scan kartu NFC → Balance transfer.
//
// User Flow:
// ┌─────────────────────────────────────────────────────────────────────┐
...
// │ 6. Alert muncul: "Scan Kartu NFC"                                   │
// │ 7. Pengguna dekatkan kartu NFC ke HP                                │
// │ 8. System baca UID kartu                                             │
// │ 9. System validate kartu (registered, active, balance cukup)        │
// │ 10. System ambil receiver card dari database (auto-detect)         │
// │ 11. System proses payment: kartu sumber → merchant                  │
...
//
// 2. NFC Payment (Receive Money):
//    - Input amount dengan validation
//    - Use usePayment hook (processTapToPayTransfer)
//    - Scan kartu NFC untuk receive payment
//    - Auto-transfer dari saldo kartu sumber → saldo penerima
//
// 3. Amount Validation:
//    - Must be number > 0
...
// Mengambil saldo dari respons user langsung atau wrapper defensif, lalu menolak nilai
// nonangka, non-finite, dan negatif agar UI tidak menampilkan saldo backend yang rusak.
export const extractUserBalance = (response: unknown): number => {
  const payload = response as { balance?: unknown; user?: { balance?: unknown } } | null;
  const balance = payload?.user?.balance ?? payload?.balance;
  if (typeof balance !== 'number' || !Number.isFinite(balance) || balance < 0) {
    throw new Error('Respons saldo backend tidak valid');
  }
  return balance;
};

// Menormalkan input Rupiah polos, bertitik ribuan, atau berkoma desimal menjadi angka positif.
// Format ambigu atau nilai non-finite ditolak dengan null sebelum request pembayaran dibuat.
export const parseRupiahInput = (rawValue: string): number | null => {
  const compact = rawValue.trim().replace(/^rp\s*/i, '').replace(/\s+/g, '');
  ...
  if (/^\d+$/.test(compact)) {
    ...
    canonicalValue = compact.replace(/\./g, '').replace(',', '.');
  } else if (/^\d+,\d+$/.test(compact)) {
    canonicalValue = compact.replace(',', '.');
  } else if (/^\d+\.\d+$/.test(compact) && !/^\d{1,3}\.\d{3}$/.test(compact)) {
    canonicalValue = compact;
  } else {
    return null;
  }
...
export default function NFCScreen({ user, onBack }: NFCScreenProps) {
  ...
  const initialBalance =
    typeof user?.balance === 'number' && Number.isFinite(user.balance) && user.balance >= 0
      ? user.balance
      : null;
  const [currentBalance, setCurrentBalance] = useState<number | null>(initialBalance);
  // user?.balance menggunakan optional chaining — aman jika user undefined; || 0 adalah fallback jika balance null/undefined
  const [balanceSyncFailed, setBalanceSyncFailed] = useState(initialBalance === null);
  ...
  //
  // Response canonical: { balance: number, ... } setelah dinormalisasi API service.
  // Wrapper { user: { balance } } tetap diterima sebagai bentuk defensif.
  //
  // Error Handling:
  ...
  const fetchBalance = async () => {
    ...
    try {
      ...
      // Request ke backend
      const refreshedBalance = extractUserBalance(response);
      setCurrentBalance(refreshedBalance);
      setBalanceSyncFailed(false);
      console.log('✅ Balance refreshed:', refreshedBalance);
    } catch (error: any) {
  ...
  const handleSendMoney = async () => {
    ...
    if (amountNum === null) {
      Alert.alert('Error', 'Masukkan nominal Rupiah positif yang valid');
      // Alert.alert() menampilkan dialog popup native kepada user; title dan message ditentukan oleh argumen
      return;
      ...
      <ScrollView style={styles.content} contentContainerStyle={styles.contentContainer}>
        <View style={styles.userCard}>
          ...
          <Text style={styles.userBalance}>
            Balance: {currentBalance === null ? 'Tidak tersedia' : `Rp ${currentBalance.toLocaleString('id-ID')}`}
          </Text>
        </View>
        {balanceSyncFailed && (
          <View style={styles.warningCard} accessibilityRole="alert">
            <Text style={styles.warningText}>
              Saldo terbaru belum dapat dimuat. Nilai yang terlihat mungkin belum diperbarui.
            </Text>
          </View>
        )}
        <View style={styles.instructionCard}>
          ...
          <Text style={styles.instructionText}>
            ...
            3. Tempelkan kartu NFC ke HP Anda{'{\n}'}
            4. Saldo Anda otomatis bertambah! ✅{'{\n}'}
            5. Saldo kartu sumber otomatis berkurang! ✅
          </Text>
        </View>
        <View style={styles.inputCard}>
          ...
          <Text style={styles.inputHint}>Masukkan jumlah (contoh: 19456,75)</Text>
        </View>
        <TouchableOpacity
