...
// Implement backend authentication with a server-issued JWT session.
//
// User Flow:
// ┌─────────────────────────────────────────────────────────────────────┐
// │ 1. User buka app                                                       │
...
// │ 6. System login via Backend API                                     │
// │    └─ Success: Get token + user data, save to AsyncStorage       │
// │    └─ Failed: Show the backend/network error                     │
...
//    - Backend API validates credentials and issues a JWT token
//    - Failed authentication never creates a local signed-in session
//
// 2. Form Validation:
...
export default function LoginScreen({ onLogin, onNavigateToRegister }: LoginScreenProps) {
  ...
  // ================================================================================
  // Main login handler menggunakan autentikasi backend.
  //
  // FLOW:
  // ┌─────────────────────────────────────────────────────────────────────┐
  ...
  // │         └─ Failed: Show error without creating a local session  │
  // ├─────────────────────────────────────────────────────────────────────┤
  // │ STEP 5: Set loading = false (finally block)                        │
  // └─────────────────────────────────────────────────────────────────────┘
  //
  // Backend adalah sumber kebenaran sesi agar akun nonaktif atau token yang
  // dicabut tidak dapat digantikan oleh autentikasi lokal yang sudah usang.
  // ================================================================================
  const handleLogin = async () => {
    ...
    try {
      ...
      // Backend akan memvalidasi username/password dan mengirim JWT jika valid.
      ...
      // Tampilkan kegagalan autentikasi atau jaringan tanpa membuat sesi lokal.
    } finally {
