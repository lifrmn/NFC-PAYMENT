...
// ==================================================================================
import React, { useState } from 'react'; // import React (wajib untuk JSX) dan useState hook untuk membuat state lokal (n...
...
import { // import beberapa komponen atau fungsi sekaligus dari satu modul menggunakan de...
  ...
  TextInput, // TextInput adalah input teks — digunakan untuk semua field form (nama, usernam...
  // TextInput adalah input teks — digunakan untuk semua field form (nama, username, password, konfirmasi)
  TouchableOpacity, // TouchableOpacity adalah area yang bisa ditekan dengan efek transparan — digun...
  // TouchableOpacity adalah area yang bisa ditekan dengan efek transparan — digunakan untuk link "Sudah punya akun?"
  Alert, // Alert menampilkan dialog popup native untuk validasi input dan pesan sukses r...
  // Alert menampilkan dialog popup native untuk validasi input dan pesan sukses registrasi
  KeyboardAvoidingView, // KeyboardAvoidingView menggeser layout secara otomatis saat keyboard muncul ag...
  // KeyboardAvoidingView menggeser layout secara otomatis saat keyboard muncul agar form tidak tertutup keyboard
  Platform, // Platform.OS mengembalikan 'android' atau 'ios' — digunakan karena perilaku ke...
  // Platform.OS mengembalikan 'android' atau 'ios' — digunakan karena perilaku keyboard berbeda di tiap platform
  ScrollView // ScrollView memungkinkan form bisa di-scroll saat konten melebihi tinggi layar...
...
import AsyncStorage from '@react-native-async-storage/async-storage'; // AsyncStorage adalah penyimpanan key-value persisten — digunakan untuk menyimp...
...
import CustomButton from '../components/CustomButton'; // import komponen CustomButton dari file lokal — tombol reusable dengan loading...
// import komponen CustomButton dari file lokal — tombol reusable dengan loading state dan variant warna
import { apiService } from '../utils/apiService'; // import apiService Singleton dari apiService.ts — digunakan untuk mengirim req...
...
// ==================================================================================
interface RegisterScreenProps { // interface adalah blueprint TypeScript untuk mendefinisikan struktur props yan...
  // interface adalah blueprint TypeScript untuk mendefinisikan struktur props yang diterima komponen RegisterScreen
  onRegisterSuccess: () => void; // callback function tanpa argumen (() => void) — dipanggil App.tsx saat registr...
  // callback function tanpa argumen (() => void) — dipanggil App.tsx saat registrasi berhasil, biasanya untuk navigasi kembali ke login
  onNavigateToLogin: () => void; // callback function tanpa argumen — dipanggil saat user menekan link "Sudah pun...
...
// ==================================================================================
export default function RegisterScreen({ onRegisterSuccess, onNavigateToLogin }: RegisterScreenProps) { // export default function: mendefinisikan dan mengekspor komponen React fungsio...
  ...
  const [name, setName] = useState(''); // const membuat variabel tetap; useState('') membuat state string kosong; name ...
  ...
  const [username, setUsername] = useState(''); // useState('') nilai awal string kosong; username digunakan sebagai identifier ...
  ...
  const [password, setPassword] = useState(''); // useState('') nilai awal string kosong; password akan dikirim ke backend untuk...
  ...
  const [confirmPassword, setConfirmPassword] = useState(''); // useState('') nilai awal string kosong; dibandingkan dengan password menggunak...
  ...
  const [loading, setLoading] = useState(false); // useState(false) nilai awal boolean false; loading=true menonaktifkan tombol D...
  ...
  // ================================================================================
  const handleRegister = async () => { // handleRegister async: memproses pendaftaran user baru; async karena request API
    ...
    if (!name.trim() || !username.trim() || !password.trim() || !confirmPassword.trim()) { // if (!...) validasi bahwa nilai tidak kosong/null sebelum melanjutkan operasi
      // if (!...) validasi bahwa nilai tidak kosong/null sebelum melanjutkan operasi
      Alert.alert('Error', 'Semua field harus diisi'); // Alert.alert() menampilkan dialog popup native kepada user; title dan message ...
    ...
    if (username.length < 3) { // validasi panjang username minimum 3 karakter; mencegah username terlalu pende...
      // validasi panjang username minimum 3 karakter; mencegah username terlalu pendek yang tidak informatif
      Alert.alert('Error', 'Username minimal 3 karakter'); // Alert.alert() menampilkan dialog popup native kepada user; title dan message ...
      // Alert.alert() menampilkan dialog popup native kepada user; title dan message ditentukan oleh argumen
    ...
    if (password.length < 6) { // validasi panjang password minimum 6 karakter; password pendek lebih rentan te...
      // validasi panjang password minimum 6 karakter; password pendek lebih rentan terhadap brute force attack
      Alert.alert('Error', 'Password minimal 6 karakter'); // Alert.alert() menampilkan dialog popup native kepada user; title dan message ...
      // Alert.alert() menampilkan dialog popup native kepada user; title dan message ditentukan oleh argumen
    ...
    if (password !== confirmPassword) { // validasi konfirmasi password: memastikan user tidak salah ketik password saat...
      // validasi konfirmasi password: memastikan user tidak salah ketik password saat registrasi
      Alert.alert('Error', 'Password dan konfirmasi password tidak sama'); // Alert.alert() menampilkan dialog popup native kepada user; title dan message ...
      // Alert.alert() menampilkan dialog popup native kepada user; title dan message ditentukan oleh argumen
    ...
    setLoading(true); // setLoading(true) menampilkan spinner; dilakukan sebelum operasi async yang me...
    ...
    try { // try: membungkus operasi yang berisiko error; jika terjadi error akan ditangka...
      ...
      console.log('📝 Attempting registration for:', username); // console.log mencetak pesan debug ke terminal; membantu melacak alur dan nilai...
      // console.log mencetak pesan debug ke terminal; membantu melacak alur dan nilai variabel
      
      // Kirim data registrasi ke backend
      const response = await apiService.register({ name, username, password }); // const response: menyimpan response dari HTTP request; await menunggu response...
      // const response: menyimpan response dari HTTP request; await menunggu response diterima

      if (response?.user) { // memeriksa apakah response dari backend berisi objek user; tanda ?. (optional ...
        // memeriksa apakah response dari backend berisi objek user; tanda ?. (optional chaining) mencegah error jika null
        if (response.token) await AsyncStorage.setItem('token', response.token); // AsyncStorage.setItem() menyimpan data ke penyimpanan lokal perangkat secara a...
        // AsyncStorage.setItem() menyimpan data ke penyimpanan lokal perangkat secara async
        if (response.user?.id) await AsyncStorage.setItem('userId', response.user.id.toString()); // AsyncStorage.setItem() menyimpan data ke penyimpanan lokal perangkat secara a...
        // AsyncStorage.setItem() menyimpan data ke penyimpanan lokal perangkat secara async
        Alert.alert('Berhasil', 'Akun berhasil dibuat! Silakan login.', [ // Alert.alert() menampilkan dialog popup native kepada user; title dan message ...
        // Alert.alert() menampilkan dialog popup native kepada user; title dan message ditentukan oleh argumen
          { text: 'OK', onPress: onRegisterSuccess }, // text 'OK' adalah label tombol; onPress memanggil callback onRegisterSuccess s...
      ...
      } else if (response?.message) { // else if: kondisi alternatif yang diperiksa jika kondisi if sebelumnya tidak t...
        // else if: kondisi alternatif yang diperiksa jika kondisi if sebelumnya tidak terpenuhi
        Alert.alert('Gagal', response.message); // Alert.alert() menampilkan dialog popup native kepada user; title dan message ...
        // Alert.alert() menampilkan dialog popup native kepada user; title dan message ditentukan oleh argumen
        return; // return tanpa nilai: menghentikan eksekusi fungsi saat ini tanpa mengembalikan...
        // return tanpa nilai: menghentikan eksekusi fungsi saat ini tanpa mengembalikan apapun
      }
      Alert.alert('Gagal', 'Registrasi gagal. Coba lagi.'); // Alert.alert() menampilkan dialog popup native kepada user; title dan message ...
      // Alert.alert() menampilkan dialog popup native kepada user; title dan message ditentukan oleh argumen
      ...
      console.error('\u274c Register error:', error); // console.error mencetak pesan error ke terminal dengan tanda merah; untuk debu...
      // console.error mencetak pesan error ke terminal dengan tanda merah; untuk debugging masalah
      Alert.alert('Error', 'Terjadi kesalahan saat membuat akun'); // Alert.alert() menampilkan dialog popup native kepada user; title dan message ...
      // Alert.alert() menampilkan dialog popup native kepada user; title dan message ditentukan oleh argumen
      ...
      setLoading(false); // setLoading(false) mengaktifkan kembali tombol Daftar setelah proses selesai a...
  ...
  // ================================================================================
  const handleNavigateToLogin = () => { // const membuat variabel tetap; arrow function () => {} tanpa async; wrapper se...
  ...
  // return mengembalikan JSX yang akan dirender ke layar
    <SafeAreaView style={styles.container}>
      <KeyboardAvoidingView
        behavior={Platform.OS === 'ios' ? 'padding' : 'height'} // Platform.OS mengembalikan 'ios' atau 'android'; ternary operator memilih beha...
        ...
        style={styles.keyboardView} // style={} menerapkan objek style yang sudah didefinisikan di StyleSheet ke ele...
          ...
          <View style={styles.content}>
            ...
            <View style={styles.form}>
              ...
              // TextInput: kolom input teks; setara dengan input di HTML; mendukung keyboard native
                ...
                placeholderTextColor="#95a5a6" // placeholderTextColor: warna teks placeholder; biasanya abu-abu agar kontras d...
                // placeholderTextColor: warna teks placeholder; biasanya abu-abu agar kontras dengan teks input normal
                value={name} // value={name} membuat TextInput menjadi controlled component — nilainya selalu...
              ...
              // TextInput: kolom input teks; setara dengan input di HTML; mendukung keyboard native
                style={styles.input} // style={} menerapkan objek style yang sudah didefinisikan di StyleSheet ke ele...
                // style={} menerapkan objek style yang sudah didefinisikan di StyleSheet ke elemen ini
                placeholder="Username" // placeholder: teks abu-abu yang ditampilkan dalam TextInput saat belum ada inp...
              ...
              // TextInput: kolom input teks; setara dengan input di HTML; mendukung keyboard native
                style={styles.input} // style={} menerapkan objek style yang sudah didefinisikan di StyleSheet ke ele...
                // style={} menerapkan objek style yang sudah didefinisikan di StyleSheet ke elemen ini
                placeholder="Password (min. 6 karakter)" // placeholder: teks abu-abu yang ditampilkan dalam TextInput saat belum ada inp...
                // placeholder: teks abu-abu yang ditampilkan dalam TextInput saat belum ada input dari user
                placeholderTextColor="#95a5a6" // placeholderTextColor: warna teks placeholder; biasanya abu-abu agar kontras d...
                // placeholderTextColor: warna teks placeholder; biasanya abu-abu agar kontras dengan teks input normal
                value={password} // value={} mengikat nilai input ke state; membuat TextInput menjadi controlled ...
                // value={} mengikat nilai input ke state; membuat TextInput menjadi controlled component
                onChangeText={setPassword} // onChangeText dipanggil setiap user mengetik; parameter berisi teks terbaru; d...
                // onChangeText dipanggil setiap user mengetik; parameter berisi teks terbaru; digunakan untuk update state
                secureTextEntry // secureTextEntry (tanpa ={true}) secara default bernilai true — menyembunyikan...
                // secureTextEntry (tanpa ={true}) secara default bernilai true — menyembunyikan karakter password
                autoComplete="password" // autoComplete: petunjuk ke sistem untuk autofill; membantu user mengisi form l...
              ...
              // TextInput: kolom input teks; setara dengan input di HTML; mendukung keyboard native
                style={styles.input} // style={} menerapkan objek style yang sudah didefinisikan di StyleSheet ke ele...
                // style={} menerapkan objek style yang sudah didefinisikan di StyleSheet ke elemen ini
                placeholder="Konfirmasi Password" // placeholder: teks abu-abu yang ditampilkan dalam TextInput saat belum ada inp...
                // placeholder: teks abu-abu yang ditampilkan dalam TextInput saat belum ada input dari user
                placeholderTextColor="#95a5a6" // placeholderTextColor: warna teks placeholder; biasanya abu-abu agar kontras d...
                // placeholderTextColor: warna teks placeholder; biasanya abu-abu agar kontras dengan teks input normal
                value={confirmPassword} // value={} mengikat nilai input ke state; membuat TextInput menjadi controlled ...
                // value={} mengikat nilai input ke state; membuat TextInput menjadi controlled component
                onChangeText={setConfirmPassword} // onChangeText dipanggil setiap user mengetik; parameter berisi teks terbaru; d...
                // onChangeText dipanggil setiap user mengetik; parameter berisi teks terbaru; digunakan untuk update state
                ...
                // autoComplete: petunjuk ke sistem untuk autofill; membantu user mengisi form lebih cepat
              />
              <CustomButton // komponen CustomButton reusable: menampilkan tombol dengan loading state ...
              // komponen CustomButton reusable: menampilkan tombol dengan loading state dan berbagai variant
                title={loading ? 'Membuat Akun...' : 'Daftar'} // ternary operator: jika loading=true tampilkan 'Membuat Akun...', jika false t...
                ...
                disabled={loading} // disabled={loading} menonaktifkan tombol saat sedang memproses untuk mencegah ...
                ...
                style={styles.registerButton} // style={} menerapkan objek style yang sudah didefinisikan di StyleSheet ke ele...
                // style={} menerapkan objek style yang sudah didefinisikan di StyleSheet ke elemen ini
              />
